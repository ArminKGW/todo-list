  ...
  document.addEventListener('click', (e) => {
    ...
    switch (target.className) {
      ...
      case 'edit-project':
        ...
        const editProjectDialog = document.querySelector(
          '#edit-project-dialog',
        );
        const inputProjectName = document.querySelector('#edit-project-name');
        inputProjectName.value = projectToEdit.textContent;
        editProjectDialog.dataset.projectDialogIndex =
          projectToEdit.dataset.column;
...
function showProject(project) {
  ...
  const tasks = projects[project.dataset.column].todoItems;
...
function editTaskInDOM(taskIndex, task) {
  ...
  const taskBox = document.querySelector(
    `.task-box[data-task-index="${taskIndex}"]`,
  );
  ...
  priority.classList.replace(
    priority.classList[0],
...
function initializeTaskForm(projectName, taskIndex) {
  ...
  const { name } = projects[projectIndex].todoItems[taskIndex];
  const { description } = projects[projectIndex].todoItems[taskIndex];
  const date = projects[projectIndex].todoItems[taskIndex].dueDate;
  const { notes } = projects[projectIndex].todoItems[taskIndex];
  const { priority } = projects[projectIndex].todoItems[taskIndex];
